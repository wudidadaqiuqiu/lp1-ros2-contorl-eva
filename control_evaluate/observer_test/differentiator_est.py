...
from .td_function import TdFunction

class PowerTryNode(Node):
    def __init__(self):
        super().__init__('differentiator_est_node')
        self.X_accumulated = []
        self.P_accumulated = []
        ...
        self.td = TdFunction(500000, 0.001, 0.002)
        self.msgii = Float32()
        self.msgoi = Float32()
        self.msg_power = Float32()
        self.msg_diff = Float32MultiArray()
        self.power = 0.0
        self.omega = 0.0
        self.current = 0.0
        ...
        self.pub_diff = self.create_publisher(Float32MultiArray, '/omega_diff', 10)
    ...
    def omega_callback(self, msg: Float32):
        self.omega = msg.data
        self.td.update(self.omega)
        self.msg_diff.data = [self.td.v1, self.td.get_diff()]
        self.pub_diff.publish(self.msg_diff)
